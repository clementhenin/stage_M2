...
data = data[(data['savings_IMF'].apply(np.absolute) < 200).values +
            (data['savings_IMF'].isnull()).values]
...
ginis = ["gini_DS", "gini_before_taxe_OECD", "gini_WIID", 'gini_net_SWIID']
...
# we limit ourselves to a subset of variables to have as much couple
# country/period
...
# making linear interpolation of years of schooling data to artificially
...
dataset.sort_index().to_csv("frankeinstein.csv")
